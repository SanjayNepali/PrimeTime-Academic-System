...
# PURPOSE: Manually trigger stress calculation for all students
# ============================================
...
class CalculateStressCommand(BaseCommand):
    help = 'Calculate stress levels for all active students'

    def handle(self, *args, **options):
        self.stdout.write(self.style.SUCCESS('🧠 Calculating stress for all students...\n'))
        ...
        for student in students:
            ...
            if result:
                ...
                status = '🔴 HIGH' if result.level >= 70 else '🟡 MEDIUM' if result.level >= 40 else '🟢 LOW'
                ...
                if result.level >= 70:
                    self.stdout.write(
                        self.style.ERROR(f'  ⚠️ HIGH STRESS ALERT for {student.display_name}!')
                    )
            else:
                skipped += 1
                self.stdout.write(
...
1. Test if algorithms are working:
   python manage.py test_algorithms

2. Calculate stress for all students:
   python manage.py calculate_all_stress
...
3. Check specific student:
   ...
   >>> analyzer = AdvancedSentimentAnalyzer(student)
   >>> result = analyzer.comprehensive_stress_analysis(days=7)
   >>> print(f"Stress: {result.level}%")

4. Check database records:
   python manage.py dbshell
   >>> SELECT * FROM analytics_stresslevel ORDER BY calculated_at DESC LIMIT 10;
   >>> SELECT id, sender_id, sentiment_score, is_flagged FROM chat_message ORDER BY timestamp DESC LIMIT 10;
"""